...
var ASM_CONSTS = {
  1220: function($0, $1) {sock = new WebSocket(UTF8ToString($0)); sock.addEventListener("message", (ev) => { ev.data.arrayBuffer().then((abuf) => { let buf = _malloc(abuf.byteLength); HEAP8.set(new Uint8Array(abuf), buf); _client_recv_cb(buf, abuf.byteLength, $1); }); }); sock.addEventListener("error", (ev) => { _client_err_cb($1); });},  
 1536: function() {return sock.readyState;},  
 1560: function($0, $1) {sock.send(HEAP8.subarray($0, $0+$1));},  
 1602: function() {sock.close();},  
 1616: function() {list_elem=[];},  
 1630: function($0) {elem = document.getElementById(UTF8ToString($0)); if (elem==null) { console.log(UTF8ToString($0)); }},  
 1735: function($0) {elem = list_elem[$0];},  
 1783: function($0) {elem.removeEventListener(UTF8ToString($0), null);},  
 1837: function($0, $1) {elem.addEventListener(UTF8ToString($0), (ev) => { if (_html_cb($1)==1) ev.preventDefault(); });},  
 1937: function() {elem.remove()},  
 1951: function() {elem=document.body;},  
 1971: function($0, $1) {list_elem[$1] = elem.appendChild(document.createElement(UTF8ToString($0)));},  
 2051: function($0, $1) {elem.appendChild(document.createElement(UTF8ToString($0))).id = UTF8ToString($1);},  
 2137: function($0) {list_elem[$0]=elem;},  
 2157: function($0) {($0 ? document.getElementById(UTF8ToString($0)) : document.body).appendChild(elem)},  
 2242: function() {elem.innerText = "";},  
 2265: function($0) {elem.innerText = UTF8ToString($0);},  
 2409: function() {let v = elem.value; let len = lengthBytesUTF8(v)+1; let buf = _malloc(len); stringToUTF8(v, buf, len); return buf;},  
 2528: function() {return elem.checked;},  
 2551: function($0) {let v = window.localStorage.getItem(UTF8ToString($0)); if (v==null) return 0; let len = lengthBytesUTF8(v)+1; let buf = _malloc(len); stringToUTF8(v, buf, len); return buf;},  
 2728: function($0, $1) {window.localStorage.setItem(UTF8ToString($0), UTF8ToString($1));},  
 2795: function($0) {elem.classList.remove(UTF8ToString($0))},  
 2837: function($0) {elem.removeAttribute(UTF8ToString($0))},  
 2878: function($0) {elem.classList.add(UTF8ToString($0))},  
 2915: function($0) {elem.setAttribute(UTF8ToString($0), "")},  
 2955: function($0, $1) {elem.setAttribute(UTF8ToString($0), UTF8ToString($1));},  
 3010: function() {Module.noExitRuntime = true;}
};
