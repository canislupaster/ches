...
var ASM_CONSTS = {
  1183: function($0, $1) {sock = new WebSocket(UTF8ToString($0)); sock.addEventListener("message", (ev) => { ev.data.arrayBuffer().then((abuf) => { let buf = _malloc(abuf.byteLength); HEAP8.set(new Uint8Array(abuf), buf); _client_recv_cb(buf, abuf.byteLength, $1); }); }); sock.addEventListener("error", (ev) => { _client_err_cb($1); });},  
 1499: function() {return sock.readyState;},  
 1523: function($0, $1) {sock.send(HEAP8.subarray($0, $0+$1));},  
 1565: function() {sock.close();},  
 1608: function() {list_elem=[];},  
 1622: function($0) {elem = document.getElementById(UTF8ToString($0)); if (elem==null) { console.log(UTF8ToString($0)); }},  
 1727: function($0) {elem = list_elem[$0];},  
 1753: function($0, $1) {window.setTimeout(()=>{ _html_cb($0); }, $1);},  
 1825: function($0) {elem.removeEventListener(UTF8ToString($0), null);},  
 1879: function($0, $1) {elem.addEventListener(UTF8ToString($0), (ev) => { if (_html_cb($1)==1) ev.preventDefault(); });},  
 1979: function() {elem.remove()},  
 1993: function() {elem=document.body;},  
 2013: function($0, $1) {list_elem[$1] = elem.appendChild(document.createElement(UTF8ToString($0)));},  
 2093: function($0, $1) {elem.appendChild(document.createElement(UTF8ToString($0))).id = UTF8ToString($1);},  
 2179: function($0) {list_elem[$0]=elem;},  
 2199: function($0) {($0 ? document.getElementById(UTF8ToString($0)) : document.body).appendChild(elem)},  
 2284: function() {elem.innerText = "";},  
 2307: function($0) {elem.innerText = UTF8ToString($0);},  
 2451: function() {let v = elem.value; let len = lengthBytesUTF8(v)+1; let buf = _malloc(len); stringToUTF8(v, buf, len); return buf;},  
 2570: function() {return elem.checked;},  
 2593: function($0) {let v = window.localStorage.getItem(UTF8ToString($0)); if (v==null) return 0; let len = lengthBytesUTF8(v)+1; let buf = _malloc(len); stringToUTF8(v, buf, len); return buf;},  
 2770: function($0, $1) {window.localStorage.setItem(UTF8ToString($0), UTF8ToString($1));},  
 2837: function($0) {elem.classList.remove(UTF8ToString($0))},  
 2879: function() {elem.value="";},  
 2896: function($0) {elem.removeAttribute(UTF8ToString($0))},  
 2937: function($0) {elem.classList.add(UTF8ToString($0))},  
 2974: function($0) {elem.value = UTF8ToString($0);},  
 3005: function($0) {elem.setAttribute(UTF8ToString($0), "")},  
 3045: function($0, $1) {elem.setAttribute(UTF8ToString($0), UTF8ToString($1));},  
 3100: function() {Module.noExitRuntime = true;}
};
