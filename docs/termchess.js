...
var ASM_CONSTS = {
  1184: function($0, $1) {sock = new WebSocket(UTF8ToString($0)); sock.addEventListener("message", (ev) => { ev.data.arrayBuffer().then((abuf) => { let buf = _malloc(abuf.byteLength); HEAP8.set(new Uint8Array(abuf), buf); _client_recv_cb(buf, abuf.byteLength, $1); }); }); sock.addEventListener("error", (ev) => { _client_err_cb($1); });},  
 1500: function() {return sock.readyState;},  
 1524: function($0, $1) {sock.send(HEAP8.subarray($0, $0+$1));},  
 1566: function() {sock.close();},  
 1580: function() {list_elem=[];},  
 1594: function($0) {elem = document.getElementById(UTF8ToString($0)); if (elem==null) { console.log(UTF8ToString($0)); }},  
 1699: function($0) {elem = list_elem[$0];},  
 1725: function($0, $1) {window.setTimeout(()=>{ _html_cb($0); }, $1);},  
 1797: function($0) {elem.removeEventListener(UTF8ToString($0), null);},  
 1851: function($0, $1) {elem.addEventListener(UTF8ToString($0), (ev) => { if (_html_cb($1)==1) ev.preventDefault(); });},  
 1951: function() {elem.remove()},  
 1965: function() {elem=document.body;},  
 1985: function($0, $1) {list_elem[$1] = elem.appendChild(document.createElement(UTF8ToString($0)));},  
 2065: function($0, $1) {elem.appendChild(document.createElement(UTF8ToString($0))).id = UTF8ToString($1);},  
 2151: function($0) {list_elem[$0]=elem;},  
 2171: function($0) {($0 ? document.getElementById(UTF8ToString($0)) : document.body).appendChild(elem)},  
 2256: function() {elem.innerText = "";},  
 2279: function($0) {elem.innerText = UTF8ToString($0);},  
 2423: function() {let v = elem.value; let len = lengthBytesUTF8(v)+1; let buf = _malloc(len); stringToUTF8(v, buf, len); return buf;},  
 2542: function() {return elem.checked;},  
 2565: function($0) {let v = window.localStorage.getItem(UTF8ToString($0)); if (v==null) return 0; let len = lengthBytesUTF8(v)+1; let buf = _malloc(len); stringToUTF8(v, buf, len); return buf;},  
 2742: function($0, $1) {window.localStorage.setItem(UTF8ToString($0), UTF8ToString($1));},  
 2809: function($0) {elem.classList.remove(UTF8ToString($0))},  
 2851: function() {elem.value="";},  
 2868: function($0) {elem.removeAttribute(UTF8ToString($0))},  
 2909: function($0) {elem.classList.add(UTF8ToString($0))},  
 2946: function($0) {elem.value = UTF8ToString($0);},  
 2977: function($0) {elem.setAttribute(UTF8ToString($0), "")},  
 3017: function($0, $1) {elem.setAttribute(UTF8ToString($0), UTF8ToString($1));},  
 3072: function() {Module.noExitRuntime = true;}
};
