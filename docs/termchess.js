...
var ASM_CONSTS = {
  1322: function($0, $1) {sock = new WebSocket(UTF8ToString($0)); sock.addEventListener("message", (ev) => { ev.data.arrayBuffer().then((abuf) => { let buf = _malloc(abuf.byteLength); HEAP8.set(new Uint8Array(abuf), buf); _client_recv_cb(buf, abuf.byteLength, $1); }); }); sock.addEventListener("error", (ev) => { _client_err_cb($1); });},  
 1638: function() {return sock.readyState;},  
 1662: function($0, $1) {sock.send(HEAP8.subarray($0, $0+$1));},  
 1704: function() {sock.close();},  
 1857: function() {list_elem=[]; tocstr=(v)=>{ let len = lengthBytesUTF8(v)+1; let buf = _malloc(len); stringToUTF8(v, buf, len); return buf; };},  
 1983: function($0) {elem = document.getElementById(UTF8ToString($0)); if (elem==null) { console.log(UTF8ToString($0)); }},  
 2088: function($0) {elem = list_elem[$0];},  
 2114: function($0, $1) {window.setTimeout(()=>{ _html_cb($0, 0); }, $1);},  
 2167: function($0, $1) {let sound = new Audio(UTF8ToString($0)); sound.volume = $1; sound.play();},  
 2245: function($0) {elem.addEventListener("dragover", (ev) => { ev.preventDefault(); }); elem.addEventListener("drop", (ev) => { ev.preventDefault(); _html_cb($0, tocstr(ev.dataTransfer.getData("text/plain"))); });},  
 2476: function($0) {elem.removeEventListener(UTF8ToString($0), null);},  
 2530: function($0, $1) {elem.addEventListener(UTF8ToString($0), (ev) => { if (_html_cb($1, 0)==1) ev.preventDefault(); });},  
 2633: function() {elem.remove()},  
 2647: function() {elem=document.body;},  
 2667: function($0, $1) {list_elem[$1] = elem.appendChild(document.createElement(UTF8ToString($0)));},  
 2747: function($0, $1) {elem.appendChild(document.createElement(UTF8ToString($0))).id = UTF8ToString($1);},  
 2833: function($0) {list_elem[$0]=elem;},  
 2853: function($0) {($0 ? document.getElementById(UTF8ToString($0)) : document.body).appendChild(elem)},  
 2938: function() {elem.innerText = "";},  
 2961: function($0) {elem.innerText = UTF8ToString($0);},  
 3174: function() {return tocstr(elem.value);},  
 3205: function($0) {for (x of document.getElementsByName(UTF8ToString($0))) { if (x.checked) return tocstr(x.value); } return 0;},  
 3318: function() {elems = Array.prototype.slice.call(elem.children);},  
 3369: function($0) {elems = Array.prototype.slice.call(document.getElementsByName(UTF8ToString($0)));},  
 3451: function($0) {while (elems.length>0) { elem = elems.shift(); if (($0 && elem.selected) || (!$0 && elem.checked)) return tocstr(elem.value); } return 0;},  
 3593: function() {return elem.checked;},  
 3616: function($0) {let v = window.localStorage.getItem(UTF8ToString($0)); if (v==null) return 0; let len = lengthBytesUTF8(v)+1; let buf = _malloc(len); stringToUTF8(v, buf, len); return buf;},  
 3793: function($0, $1) {window.localStorage.setItem(UTF8ToString($0), UTF8ToString($1));},  
 3860: function($0) {elem.classList.remove(UTF8ToString($0))},  
 3902: function() {elem.value="";},  
 3919: function() {elem.addEventListener("dragstart", null); elem.setAttribute("draggable", "false");},  
 4006: function($0) {elem.removeAttribute(UTF8ToString($0))},  
 4047: function($0) {elem.classList.add(UTF8ToString($0))},  
 4084: function($0) {elem.value = UTF8ToString($0);},  
 4115: function($0) {if ($0!=0) { var s = UTF8ToString($0); elem.addEventListener("dragstart", (ev) => { ev.dataTransfer.setData("text/plain", s); }); } elem.setAttribute("draggable", "true");},  
 4291: function($0) {elem.setAttribute(UTF8ToString($0), "")},  
 4331: function($0, $1) {elem.setAttribute(UTF8ToString($0), UTF8ToString($1));},  
 4386: function() {loadedUI();},  
 4398: function() {Module.noExitRuntime = true;}
};
...
/** @type {function(...*):?} */
var _client_recv_cb = Module["_client_recv_cb"] = createExportWrapper("client_recv_cb");

/** @type {function(...*):?} */
var _client_err_cb = Module["_client_err_cb"] = createExportWrapper("client_err_cb");

/** @type {function(...*):?} */
var _htonl = Module["_htonl"] = createExportWrapper("htonl");

/** @type {function(...*):?} */
var _html_cb = Module["_html_cb"] = createExportWrapper("html_cb");
